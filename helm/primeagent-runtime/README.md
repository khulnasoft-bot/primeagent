# Primeagent runtime chart

Deploy Primeagent flows to Kubernetes with this Helm chart.
...
downloadFlows:
  flows:
    - url: https://raw.githubusercontent.com/datastax/primeagent-charts/main/examples/flows/basic-prompting-hello-world.json
      endpoint: hello-world
```

2. **Packaging the flow as docker image**: You can add a flow from to a docker image based on Primeagent runtime and refer to it in the chart.
...
```
kubectl create secret generic primeagent-secrets --from-literal=openai-key=sk-xxxx
```
This command will create a secret named `primeagent-secrets` with the key `openai-key` containing your secret value.

We need to create a custom `values.yaml` file to:
1. Refer to the flow we want to deploy
2. Plug the secret we created to the Primeagent deployment
...
env:
  ...
  - name: OPENAI_API_KEY
    valueFrom:
      secretKeyRef:
        name: primeagent-secrets
        key: openai-key
```
See the full file at [basic-prompting-hello-world.yaml](https://raw.githubusercontent.com/datastax/primeagent-charts/main/examples/flows/primeagent-runtime/basic-prompting-hello-world.yaml)
...
helm repo update
helm install primeagent-runtime primeagent/primeagent-runtime --values custom-values.yaml
```
...
## Upgrade Primeagent version
To change the Primeagent version or use a custom docker image, you can modify the `image` parameter in the chart.

```yaml
image:
  repository: "khulnasoft/primeagent-backend"
  tag: 1.x.y
```
...
## Primeagent secrets
The `env` section in the `values.yaml` file allows you to set environment variables for the Primeagent deployment.
...
```
kubectl create secret generic primeagent-secrets --from-literal=openai-key=sk-xxxx --from-literal=astra-token=AstraCS:xxx
```
