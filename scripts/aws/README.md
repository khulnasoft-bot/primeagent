# Deploy Primeagent on AWS

**Duration**: 30 minutes

## Introduction

In this tutorial, you will learn how to deploy primeagent on AWS using [AWS Cloud Development Kit](https://aws.amazon.com/cdk/?nc2=type_a) (CDK).
...
# How to set up your environment and deploy primeagent
...
1. Run the following commands in Cloudshell:
   ...
   cat params.json | jq '.name |= "c9-for-primeagent"'
   ./bin/bootstrap
   ```
1. When you see `Done!` in Cloudshell, open `c9-for-primeagent` from [AWS Cloud9](https://us-east-1.console.aws.amazon.com/cloud9control/home?region=us-east-1#/).
...
1. Access the URL displayed.
   ```shell
   Outputs:
   PrimeagentAppStack.frontendURLXXXXXX = https://XXXXXXXXXXX.cloudfront.net
   ```
1. Enter your user name and password to sign in. If you have not set a user name and password in your `.env` file, the user name will be set to `admin` and the password to `123456`.
   ![signin-primeagent](./img/primeagent-signin.png)
...
1. Run the following command in the Cloud9 terminal.
   ...
   ```
1. Open [AWS CloudFormation](https://us-east-1.console.aws.amazon.com/cloudformation/home?region=us-east-1#/getting-started), select `aws-cloud9-c9-for-primeagent-XXXX` and delete it.
   ![delete-cfn](./img/primeagent-cfn.png)
   s
