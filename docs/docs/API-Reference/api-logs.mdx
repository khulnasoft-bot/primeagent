...
To enable log retrieval, include set the following [environment variables](/environment-variables) in your Primeagent `.env` file, and then start Primeagent with `uv run primeagent run --env-file .env`:
