...
* [Connect to a non-Primeagent MCP server](#mcp-stdio-mode) with a JSON configuration file, server start command, or SSE URL to access tools provided by external, non-Primeagent MCP servers.
* [Connect to a Primeagent MCP server](#mcp-sse-mode) to use flows from your [Primeagent projects](/concepts-flows#projects) as MCP tools.
...
2. In the **MCP Server** field, select a previously connected server or click <Icon name="Plus" aria-hidden="true"/> **Add MCP Server**.
    ...
    * **SSE**: Enter your Primeagent MCP server's **Name**, **SSE URL**, and any **Headers** and **Environment Variables** the server uses, and then click **Add Server**.
...
### Connect a Primeagent MCP server {#mcp-sse-mode}
...
2. In the **MCP SSE URL** field, modify the default address to point at your Primeagent server's SSE endpoint. The default value for other Primeagent installations is `http://localhost:7860/api/v1/mcp/sse`.
...
| sse_url | String | Input parameter. SSE mode only. The SSE URL for a Primeagent project's MCP server. Default for Primeagent Desktop: `http://localhost:7868/`. Default for other installations: `http://localhost:7860/api/v1/mcp/sse` |
