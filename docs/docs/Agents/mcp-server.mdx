...
* A [Primeagent project](/concepts-flows#projects) with at least one flow that has a [**Chat Output** component](/components-io#chat-output).
...
Each [Primeagent project](/concepts-flows#projects) has an MCP server that exposes the project's flows as tools for use by MCP clients.
...
For example:
    ...
    The **MCP Server** tab automatically populates the `PROJECT_NAME`, `PRIMEAGENT_SERVER_ADDRESS`, and `PROJECT_ID` values.
    ...
    <Tabs groupId="auth-type">
      <TabItem value="API key" label="API key" default>
        ...
        - **Arguments**: Enter the following list of arguments, separated by spaces. Replace the values for `YOUR_API_KEY`, `PRIMEAGENT_SERVER_ADDRESS`, and `PROJECT_ID` with the values from your Primeagent MCP server. For example:
            ```bash
            mcp-proxy --headers x-api-key YOUR_API_KEY http://PRIMEAGENT_SERVER_ADDRESS/api/v1/mcp/project/PROJECT_ID/sse
            ```
      ...
      <TabItem value="None" label="None">
        ...
        - **URL**: Enter the Primeagent MCP server's `sse` endpoint. For example:
            ```bash
