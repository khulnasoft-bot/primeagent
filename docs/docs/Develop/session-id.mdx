...
To retrieve messages from local Primeagent memory, add a [**Message History** component](/components-helpers#message-history) to your flow.
...
Messages can be retrieved by `session_id` from the Primeagent API at `GET /v1/monitor/messages`. For more information, see [Monitor endpoints](https://primeagent-docs.khulnasoft.com/api-monitor).
