...
Files uploaded to Primeagent file management are stored locally in your [Primeagent configuration directory](/memory), and they are available to all of your flows.
Local storage is set by `PRIMEAGENT_STORAGE_TYPE`, which has only one allowed value (`local`).
...
To use files in your Primeagent file management system in a flow, add a component that accepts file input to your flow, such as the **File** component.
