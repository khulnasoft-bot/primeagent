...
Custom components extend Primeagent's functionality through Python classes that inherit from `Component`. This enables integration of new features, data manipulation, external services, and specialized tools.

In Primeagent's node-based environment, each node is a "component" that performs discrete functions. Custom components are Python classes which define:
...
* `icon`: A decorative icon from Primeagent's icon library, printed next to the name.
...
A Primeagent custom component is more than a class with inputs and outputs. It includes an internal structure with optional lifecycle steps, output generation, front-end interaction, and logic organization.

A basic component:

* Inherits from `primeagent.custom.Component`.
...
from primeagent.template import Output
...
By default, Primeagent looks for custom components in the `/components` directory.

If you're creating custom components in a different location using the `PRIMEAGENT_COMPONENTS_PATH` [environment variable](/environment-variables), components must be organized in a specific directory structure to be properly loaded and displayed in the visual editor:
...
This folder structure is required for Primeagent to properly discover and load your custom components. Components placed directly in the base directory aren't loaded.
...
An input is usually an instance of a class from `primeagent.io` (such as `StrInput`, `DataInput`, or `MessageTextInput`). The most common constructor parameters are:
...
```python
from primeagent.io import StrInput, BoolInput, DropdownInput
...
An output is usually an instance of `Output` from `primeagent.io`, with common parameters:
...
Primeagent currently supports the following input types for **Tool Mode**:
...
* **Validation**: Primeagent blocks incompatible connections automatically.
...
In **Primeagent**, dynamic fields allow inputs to change or appear based on user interactions. You can make an input dynamic by setting `dynamic=True`.
...
```python
from primeagent.io import DropdownInput, StrInput
...
When a field with `real_time_refresh=True` is modified, Primeagent calls the `update_build_config` method, passing the updated field name, value, and the component's configuration to dynamically adjust the visibility or properties of other fields based on user input.
...
In Primeagent, robust error handling ensures that your components behave predictably, even when unexpected situations occur, such as invalid inputs, external API failures, or internal logic errors.

### Error handling techniques

* **Raise Exceptions**: If a critical error occurs, you can raise standard Python exceptions such as `ValueError`, or specialized exceptions like `ToolException`. Primeagent will automatically catch these and display appropriate error messages in the visual editor, helping users quickly identify what went wrong.
...
* **Stop specific outputs with `self.stop(...)`**: You can halt individual output paths when certain conditions fail, without affecting the entire component. This is especially useful when working with components that have multiple output branches.
    ...
    if <some condition>:
        self.stop("some_output")  # Tells Primeagent no data flows
...
See [How to Contribute](/contributing-components) to contribute your custom component to Primeagent.