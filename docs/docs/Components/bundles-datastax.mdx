...
For more information, see [Use Primeagent agents](/agents).
...
| Is Metadata | When loading data using LangChain or Primeagent, additional attributes may be stored under a metadata object. If the target attribute is stored this way, enable this option. It adjusts the query by generating a filter in the format: `{"metadata.<attribute_name>": "<value>"}` |
