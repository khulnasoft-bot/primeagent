...
For help with creating this script, use the [Primeagent File Upload Utility](https://primeagent-file-upload-examples.onrender.com/).
...
const runRes = await fetch('PRIMEAGENT_SERVER_ADDRESS/api/v1/run/FLOW_ID', {
  method: 'POST',
  headers: { 'Content-Type': 'application/json', 'x-api-key': 'PRIMEAGENT_API_KEY' },
  body: JSON.stringify(payload)
});
const primeagentData = await runRes.json();
// Output only the message
console.log(primeagentData.outputs?.[0]?.outputs?.[0]?.results?.message?.data?.text);
```
