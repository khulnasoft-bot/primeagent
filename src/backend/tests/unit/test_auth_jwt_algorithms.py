...
class TestTokenCreation:
    ...
    def test_create_token_hs256(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch("primeagent.services.auth.utils.get_settings_service", return_value=mock_service):
                token = create_token(
...
class TestTokenVerification:
    ...
    async def test_verify_hs256_token_success(self):
        ...
        from primeagent.services.auth.utils import create_token, get_current_user_by_jwt
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with (
                patch("primeagent.services.auth.utils.get_settings_service", return_value=mock_service),
                patch("primeagent.services.auth.utils.get_user_by_id", return_value=mock_user),
            ):
                token = create_token(
                    data={"sub": "user-123", "type": "access"},
                    expires_delta=timedelta(hours=1),
...
class TestAuthenticationFailures:
    ...
    async def test_missing_public_key_rs256_raises_401(self):
        ...
        from primeagent.services.auth.utils import get_current_user_by_jwt
    ...
    async def test_user_not_found_raises_401(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with (
                patch("primeagent.services.auth.utils.get_settings_service", return_value=mock_service),
                patch("primeagent.services.auth.utils.get_user_by_id", return_value=None),
            ):
                token = create_token(
